...
pro build_env_sample
    ...
    ; SDSS
    sdss_parent = mrdfits(parentdir+'mf_parent_sdss.fits.gz',1)
    ...
    sdss_polyarea = get_poly_area(/sdss, /sr)
    h100 = mf_h100()
    
    redbin = [0.9,0.7,0.5,0.3,0.1] 
    for j=0L,n_elements(redbin)-1L do begin 
        bin_zmin = redbin[j]-0.1 
        bin_zmax = redbin[j]+0.1 

        if (redbin[j] EQ 0.1) then begin 
            bin_zmin    = 0.06
            bin_zmax    = 0.145
            zbin_indx   = sdss_parent.zprimus GE bin_zmin AND sdss_parent.zprimus LT bin_zmax
            bin         = sdss_parent[where(zbin_indx,zbin_count)]
            polyarea    = get_poly_area(/sdss, /sr) 
        endif else begin 
        ...
        bin_g           = bin.K_FNUVUGRIZJHK_ABSMAG_01[3]
        ...
        for ii=0L,n_elements(bin)-1L do printf,lun,bin_g[ii],bin[ii].zprimus,bin_vmax_evol[ii]
        free_lun,lun

        bin_hist    = hist_smf(bin_g,bin_vmax_evol,weight=bin_weights, binsize=0.01)
        ...
        if (redbin[j] EQ 0.1) then begin 
            absmag_lim_indx = sdss_parent.K_FNUVUGRIZJHK_ABSMAG_01[3] le bin_absmag_lim
            sdss_parent[where(zbin_indx AND absmag_lim_indx)].comp = 1L 
        endif else begin 
            absmag_lim_indx = primus_parent.K_FNUVUGRIZJHK_ABSMAG_01[3] le bin_absmag_lim 
            primus_parent[where(zbin_indx AND absmag_lim_indx)].comp = 1L
        endelse 
        bin_absmag_lim_indx = bin_g le bin_absmag_lim 
        ...
        for ii=0L,n_elements(bin)-1L do  begin 
            printf,lun,bin[ii].K_FNUVUGRIZJHK_ABSMAG_01[3],bin[ii].zprimus,bin[ii].vmax_evol
        endfor
        free_lun,lun
    endfor
;We impose the constraints for our sample above from above. 
    ...
    outputs = replicate({ra:0.D,dec:0.D,zprimus:0.,weight:0.,Mg:0.}, n_elements(primus_parent))
    outputs.ra      = primus_parent.ra
    outputs.dec     = primus_parent.dec
    outputs.zprimus = primus_parent.zprimus
    outputs.weight  = primus_parent.targ_weight
    outputs.Mg      = primus_parent.K_FNUVUGRIZJHK_ABSMAG_01[3] 
    mwrfits, outputs, dir+'EDP-primus-z0210-numden.fits', /create

    sdss_output = replicate({ra:0.D,dec:0.D,zprimus:0.,weight:0.,Mg:0.}, n_elements(sdss_parent))
    sdss_output.ra      = sdss_parent.ra
    sdss_output.dec     = sdss_parent.dec
    sdss_output.zprimus = sdss_parent.zprimus
    sdss_output.weight  = sdss_parent.final_weight
    sdss_output.Mg      = sdss_parent.K_FNUVUGRIZJHK_ABSMAG_01[3]
    mwrfits, sdss_output, dir+'EDP-sdss-z006_0145-numden.fits', /create
end
