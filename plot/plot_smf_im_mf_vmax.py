...
#redshift = ["$0.8<z<1.0$","$0.6<z<0.8$","$0.4<z<0.6$","$0.2<z<0.4$","$0.06<z<0.145$"]
#color = ['k','b','m','r','g']
...
for i in range(len(sfq)):
    for ii in range(len(ebin)):
        ...
        for iii in range(len(zbin)):
            ...
            if iii==len(zbin)-1: 
                fname = 'smf_sdss_cylr'+sdss_rad+'h'+sdss_height+'_thresh'+sdss_thresh+'_nbin'+sdss_nbin+'_lit_'+sfq[i]+'_'+ebin[ii]+'_nobinz.fits'
            print fname
...
for j in range(len(ebin))[::-1]:
    ...
    for jj in range(len(zbin))[::-1]:
        ...
        if jj==len(zbin)-1:
            qf_fname = 'qf_sdss_cylr'+sdss_rad+'h'+sdss_height+'_thresh'+sdss_thresh+'_nbin'+sdss_nbin+'_lit_'+ebin[j]+'_nobinz.fits'
        print qf_fname
...
#        qfzero = (qfqf > 0.0) & (qfqf < 1.0) 
#        if jj==len(zbin)-1: 
        qfzero = (qfqf > 0.0) & (qfqf < 1.0) & (qfmass > 10.0) & (qfmass < 11.5)
        qfno0 = qfqf[qfzero]
...
qffitax.set_ylabel(r'Linear fit value at $\rm{log}(\mathcal{M}_{\rm{fid}}/\mathcal{M}_{\odot})=$'+str(fiducial_mass),fontsize=18)
